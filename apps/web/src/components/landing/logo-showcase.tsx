    ...
    <section className="relative pt-8 pb-16 overflow-hidden bg-slate-50">
      <div className="relative container mx-auto px-6 max-w-7xl">
        ...
        <div className="text-center mb-12">
          <p className="text-base font-sans font-light text-slate-500">
            Made by talent from
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-12 flex-wrap">
          {LOGOS.map((logo) => (
            <div
              key={logo.name}
              className="flex items-center justify-center h-16 w-40 transition-opacity duration-300 hover:opacity-80"
            >
              <Image
                src={logo.src}
                alt={logo.name}
                width={160}
                height={64}
                className={`h-auto w-auto object-contain ${logo.name === "Chapter One" ? "max-h-16" : "max-h-12"}`}
